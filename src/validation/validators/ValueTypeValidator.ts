...
export function isValueType(valueType: string, errorMessage?: StringFormatType) {
    ...
    return (target: Object, propertyKey: string | symbol, descriptor: TypedPropertyDescriptor<any>) => {
        ...
        Reflect.defineMetadata(VALIDATOR_METADATA_KEY,
             new ValueTypeValidator(valueType, errorMessage), target, propertyKey);
    };
}

export class ValueTypeValidatorBase extends ValidatorBase {

    constructor(name: string, private valueType: string,
    ...
    private isMatchedType(value: any) {
        ...
        if (typeof value === this.valueType) return true;
            ...
            && value.constructor.name === this.valueType) return true;
        else return false ;
    }

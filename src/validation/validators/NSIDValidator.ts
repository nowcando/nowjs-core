...
export function isNSID(style = 'universal', errorMessage?: StringFormatType) {
...
export const PATTERN_NSID_UNIVERSAL = /^\d{10}$/gi;
export const PATTERN_NSID_US = /^(\+\d{1,3}[- ]?)?\d{10}$/gi;
export const PATTERN_NSID_IR = /^\d{10}$/gi;
...
export class NSIDValidator extends PatternValidatorBase {
    constructor(
        ...
        switch (style.toLocaleLowerCase()) {
            case 'universal':
                this.setPattern(PATTERN_NSID_UNIVERSAL);
                break;
            case 'ir':
                this.setPattern(PATTERN_NSID_IR);
                break;
            case 'iq':
                this.setPattern(PATTERN_NSID_IQ);
                break;
            case 'sa':
                this.setPattern(PATTERN_NSID_EN);
                break;
            case 'af':
                this.setPattern(PATTERN_NSID_AF);
                break;
            case 'ue':
                this.setPattern(PATTERN_NSID_UE);
                break;
            case 'en':
                this.setPattern(PATTERN_NSID_EN);
                break;

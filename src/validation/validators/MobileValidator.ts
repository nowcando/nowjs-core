...
export function isMobile(style = 'universal', errorMessage?: StringFormatType) {
...
export const PATTERN_MOBILE_UNIVERSAL = /^(\+\d{1,3}[- ]?)?\d{10}$/gi;
export const PATTERN_MOBILE_US = /^(\+\d{1,3}[- ]?)?\d{10}$/gi;
...
export class MobileValidator extends PatternValidatorBase {
    constructor(
        ...
        switch (style.toLocaleLowerCase()) {
            case 'universal':
                this.setPattern(PATTERN_MOBILE_UNIVERSAL);
                break;
            case 'ir':
                this.setPattern(PATTERN_MOBILE_IR);
                break;
            case 'iq':
                this.setPattern(PATTERN_MOBILE_IQ);
                break;
            case 'sa':
                this.setPattern(PATTERN_MOBILE_EN);
                break;
            case 'af':
                this.setPattern(PATTERN_MOBILE_AF);
                break;
            case 'ue':
                this.setPattern(PATTERN_MOBILE_UE);
                break;
            case 'en':
                this.setPattern(PATTERN_MOBILE_EN);
                break;

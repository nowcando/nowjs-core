...
const objectComparator: Comparator<Record<string, any>, Record<string, any>> = (a, b) => {
    if (a > b) {
        return 1;
    } else if (a === b) {
        return 0;
...
function isEquals<T, U>(x: T, y: U): boolean {
    ...
    if ((x as any) === y || x.valueOf() === y.valueOf()) {
        return true;
    }
    if (Array.isArray(x) && x.length !== (y as any).length) {
        return false;
    }
    ...
    return Object.keys(y).every(i => p.indexOf(i) !== -1) && p.every(i => isEquals((x as any)[i], (y as any)[i]));
}

export { numberComparator, stringComparator, deepObjectComparator, booleanComparator, objectComparator, isEquals };

...
export class Enumerable<T> implements IQueryable<T> {
  ...
  public static from<TSource>(
    enumerable: IEnumerable<TSource> | Iterable<TSource>
  ): IQueryable<TSource> {
    return new Enumerable(enumerable);
  }
  // tslint:disable-next-line:ban-types
  public static repeat<TSource>(
    result: TSource | Function,
    count: number
  ): IQueryable<TSource> {
    const itr = {
      [Symbol.iterator]: () => {
  ...
  // protected arr: T[] = [];
  constructor(
    private enumerable: IEnumerable<any> | Iterable<any>,
    private options?: {}
  ) {
  ...
  public join<TOuter, TInner, TKey, TResult>(
    ...
    const itr = {
      [Symbol.iterator]: () => {
        ...
        let enbg: any;
        const getGroup = (
          groups: IEnumerable<IGroup<TKey, TInner>>,
          key: TKey
        ) => {
          if (!enbg) {
  ...
  public median(selector?: Func<number, T>, predicate?: Predicate<T>): number {
    ...
    if (count % 2 === 0) {
      return (
        ((selector
          ? selector(source.get(mindex))
          : (source.get(mindex) as any)) +
          (selector
            ? selector(source.get(mindex - 1))
            : (source.get(mindex - 1) as any))) /
        2
      );
    }
    return selector

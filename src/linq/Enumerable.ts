...
export class Enumerable<T> implements IQueryable<T> {
    ...
    public static repeat<TSource>(result: TSource | Function, count: number): IQueryable<TSource> {
        const itr = {
            [Symbol.iterator]: () => {
                let counter = 0;
                return {
                                ...
                                value:
                                    typeof result === 'function'
                                        ? // tslint:disable:object-literal-sort-keys
                                          // tslint:disable-next-line:ban-types
                                          (result as Function).apply(result)
                                        : result,
                                done: false,
                            };
                        } else {
    ...
    public static range(start: number, count: number, step = 1): IQueryable<number> {
        const itr = {
            [Symbol.iterator]: () => {
                let curr = start - 1;
                let counter = 0;
    ...
    // protected arr: T[] = [];
    constructor(private enumerable: IEnumerable<any> | Iterable<any>, private options?: {}) {
    ...
    public selectMany<TCollection, TResult>(
        selector: Func<IEnumerable<TResult>, T>,
        collectionSelector?: Func<TResult, TCollection>,
    ): IQueryable<TResult> {
        const thisme = this;
    ...
    public orderBy<TSelected>(selector: Func<TSelected, T>, comparator?: Comparator<T, T>): IOrderedQueryable<T> {
    ...
    public orderByDesc<TSelected>(selector: Func<TSelected, T>, comparator?: Comparator<T, T>): IOrderedQueryable<T> {
        comparator = comparator || (Enumerable.comparator as any);
    ...
    private internalGroupBy<TKey>(
        source: any,
        keySelector: Func<TKey, T>,
        parentKey?: any,
    ): IQueryable<IGroup<TKey, T>> {
            ...
            if (item.key && item.values) {
                ...
                for (const gitem of gitems) {
                    ...
                    if (s) {
                        ...
                        groups.set(gitem.key, gitem.values as any);
                    }
                }
            } else {
                const key = parentKey ? keySelector({ key: item } as any) : keySelector(item);
                let flist = null;
        ...
        const list: any[] = [];
        groups.forEach((v, k) => {
            list.push({ key: k, values: new Enumerable(v) } as any);
        });
    ...
    public covariance(other: IQueryable<T>, selector?: Func<number, T>, predicate?: Predicate<T>): number {
        ...
        let ret = 0;
        // tslint:disable-next-line:curly
        for (let i = 0; i < sourcecount; i++) ret += (source.get(i) - sourceAvg) * ((other.get(i) as any) - targetAvg);
    ...
    public median(selector?: Func<number, T>, predicate?: Predicate<T>): number {
        ...
        if (count % 2 === 0) {
            return (
                ((selector ? selector(source.get(mindex)) : (source.get(mindex) as any)) +
                    (selector ? selector(source.get(mindex - 1)) : (source.get(mindex - 1) as any))) /
                2
            );
        }
...
export class OrderedEnumerable<T> extends Enumerable<T> implements IOrderedQueryable<T> {
    // private _array:any[];
    constructor(enumerable: IEnumerable<any> | Iterable<any>, options?: {}) {
